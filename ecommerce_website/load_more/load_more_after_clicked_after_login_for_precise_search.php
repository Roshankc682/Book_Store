...
$load_more_after_clicked_after_login_for_precise_search = $_POST['load_more_after_clicked_after_login_for_precise_search'];

// echo '<script>alert(';echo $load_more_after_clicked_after_login_for_precise_search; echo ')</script>';
   ...
   // ======connected to book where image name author_name uploaded==========';
      ...
      $index7="category";
          $quanity_check_loop = 1;
              ...
              $search =  $_SESSION["search_by_user_from_bar"];
                // echo $search;


            $search = preg_replace("#[^0-9a-z]i#","", $search);

            $query = mysqli_query($con,"SELECT * FROM book WHERE title LIKE '%$search%' OR author LIKE '%$search%' OR category LIKE '%$search%' LIMIT $load_more_after_clicked_after_login_for_precise_search") or die ("Could not search");
