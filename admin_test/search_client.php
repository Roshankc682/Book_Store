...
if(isset($_POST['search_book_from_review'])) 
{
	...
	exit();
			...
			if(isset($_GET['search_order_from_view'])) 
			{
				$search = $_GET['search_order_from_view'];
	    		$search = preg_replace("#[^0-9a-z]i#","", $search);


				$search = stripslashes($_GET['search_order_from_view']);
			 	$search = mysqli_real_escape_string($data_base_for_details,$search); 

	    		 $con = mysqli_connect('localhost','root','','bs');
	    		$query = mysqli_query($con,"SELECT * FROM user_deliver WHERE hash_token_for_client_for_ship LIKE '%$search%' OR stripe_token LIKE '%$search%' OR user_id LIKE '%$search%'  OR full_name LIKE '%$search%'  OR book_name LIKE '%$search%'  OR city LIKE '%$search%'  OR stripeToken LIKE '%$search%'  OR process LIKE '%$search%'  OR Book_stock_to_revoke LIKE '%$search%'  OR stripeEmail LIKE '%$search%'  OR time LIKE '%$search%' ") or die ('<script>window.location.replace("http://localhost/admin_test/order.php?activities=illegal");</script>');
	    		$count = mysqli_num_rows($query);
